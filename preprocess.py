...
def shuffle_together(data_set):
    xlist = list(range(data_set[0].shape[0]))  # total number of samples
    random.shuffle(xlist)
    new_data_set = []
    for module in range(len(data_set)):  # number of data modules
        newdata = data_set[module].copy()
        for dataIndex in xlist:
            newdata[dataIndex] = data_set[module][dataIndex]
...
def generate_sample_spine(purpose='train', nsamples_patch=5, nsamples_block=10, patchSize=572, patchSize_out=388, patchZ=23, patchZ_out=1, block_name='train', doAugmentation=False):
    ...
    img = imageio.imread(img_files_gray[0])  # read the first image to get information about the shape
    grayImages = np.zeros((len(img_files_gray), img.shape[0], img.shape[1]))
    membraneImages = np.zeros((len(img_files_gray), img.shape[0], img.shape[1]))

    read_order = list(range(len(img_files_gray)))
    for img_index in read_order:
        img = imageio.imread(img_files_gray[img_index])
        img = normalizeImage(img)
        img = img - 0.5
        grayImages[img_index, :, :] = img
        membrane_img = imageio.imread(img_files_membrane[img_index]) / 255.
    ...
    pickIndex = random.sample(range(len(img_files_gray) - patchZ + 1), nsamples_block)
    num_total = 0
    for i in pickIndex:
...
def prediction_full_patch_spine(patchSize=572, patchSize_out=388, patchZ=23, patchZ_out=1, writeImage=True, returnValue=True):
    ...
    with open('3d_unet_spine.json', 'r') as file:
    ...
    img = mahotas.imread(img_files_gray[0])  # Read the first image to get information about the shape
    grayImages = np.zeros((len(img_files_gray), img.shape[0], img.shape[1]))
    labelImages = np.zeros((len(img_files_gray), img.shape[0], img.shape[1]), dtype=np.int8)
    probImages = np.zeros((len(img_files_gray) - 2 * csZ, img.shape[0] - 2 * cropSize, img.shape[1] - 2 * cropSize))

    print('Total number of full size test images:', len(img_files_gray))
    for img_index in range(len(img_files_gray)):
        img = mahotas.imread(img_files_gray[img_index])
        img = normalizeImage(img)
        img -= 0.5
        grayImages[img_index, :, :] = img
        img_label = mahotas.imread(img_files_membrane[img_index]) // 255
        labelImages[img_index, :, :] = img_label.astype(np.int8)

    numSample_axis = (img.shape[0] - 2 * cropSize) // patchSize_out + 1
    numSample_patch = numSample_axis ** 2
    numZ = math.ceil((len(img_files_gray) - 2 * csZ) / patchZ_out)
    ...
    for zIndex in range(numZ):
        zStart = numProb - patchZ_out if zIndex == numZ - 1 else patchZ_out * zIndex
    ...
    for zIndex in range(numZ):
        ...
        for xIndex in range(numSample_axis - 1):
            xStart = patchSize_out * xIndex
            probImages[zStart:zStart + patchZ_out, xStart:xStart + patchSize_out, yStart:] = membrane_set[num_total, :, :, yStart - img.shape[1] + 2 * cropSize:]
            num_total += 1

        xStart = (numSample_axis - 1) * patchSize_out
        yStart = (numSample_axis - 1) * patchSize_out
        probImages[zStart:zStart + patchZ_out, xStart:, yStart:] = membrane_set[num_total, :, xStart - img.shape[0] + 2 * cropSize:, yStart - img.shape[1] + 2 * cropSize:]
        num_total += 1

    if writeImage:
        print('Store images')
        for imgIndex in range(numProb):
            scipy.misc.imsave(pathPrefix + "result/prediction_" + str(f"{imgIndex:04d}") + ".tif", probImages[imgIndex])

    end_time = time.time()
    total_time = end_time - start_time

    print('Running time:', total_time / 60, 'minutes')
